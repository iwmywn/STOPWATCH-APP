    ...
    Interval = setInterval(() => {
      ...
      if (i < 9 && j < 10) {
        if (z > 0) {
          timerElement.innerHTML = `${z}:0${j}:0${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:0${j}:0${i}`);
        }
        else {
          timerElement.innerHTML = `0${j}:0${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `0${j}:0${i}`);
        }
      } else if (i < 9 && j >= 10) {
        if (z > 0) {
          timerElement.innerHTML = `${z}:${j}:0${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:${j}:0${i}`);
        }
        else {
          timerElement.innerHTML = `${j}:0${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${j}:0${i}`);
        }
      } else if (i >= 9 && i < 59 && j < 10) {
        if (z > 0) {
          timerElement.innerHTML = `${z}:0${j}:${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:0${j}:${i}`);
        }
        else {
          timerElement.innerHTML = `0${j}:${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `0${j}:${i}`);
        }
      } else if (i >= 9 && i < 59 && j >= 10) {
        if (z > 0) {
          timerElement.innerHTML = `${z}:${j}:${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:${j}:${i}`);
        }
        else {
          timerElement.innerHTML = `${j}:${++i}`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${j}:${i}`);
        }
      } else if (i === 59 && j < 9) {
        i = 0;
        if (z > 0) {
          ...
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:0${j}:00`);
        }
        else {
          timerElement.innerHTML = `0${++j}:00`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `0${j}:00`);
        }
        ...
        if (z > 0) {
          ...
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:${j}:00`);
        }
        else {
          timerElement.innerHTML = `${++j}:00`;
          if (lap === 0)
            displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${j}:00`);
        }
      } else {
        i = 0;
        j = 0;
        ...
        if (lap === 0)
          displayLapElement.innerHTML = addRowLap(displayLapElement, 1, `${z}:00:00`);
      }
      saveToLocalStorage(i, j, z);
      localStorage.setItem('displayLap', displayLapElement.innerHTML);
      localStorage.setItem('lap', JSON.stringify(1));
    }, 1000);
  } else {
